...
public abstract class BaseDownload : BaseElement
{
    public BaseDownload(char storageDevice)
    {
...
///     ~DGd:o.x,t,w,data
/// </summary>
public class BaseDownloadGraphics : BaseDownload
{
    public BaseDownloadGraphics(char storageDevice, string imageName, string extension, Bitmap image)
        : base(storageDevice)
    {
        ImageName = imageName;
        Extension = extension;
    ...
    public override IEnumerable<string> Render(ZPLRenderOptions context)
    {
        ...
        for (var row = 0; row < contextImage.Height; row++)
        {
            ...
            for (var col = 0; col < contextImage.Width; col++)
                rowBinary += contextImage.GetPixel(col, row).GetBrightness() >= 0.5
                    ? "0"
                    : "1";
