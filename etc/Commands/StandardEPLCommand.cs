...
using System.Text;
using ZebraParser.Parser.Printing;

#endregion

namespace ZebraParser.Parser.Commands
{
    public partial class EPLCommands
    {
        public static byte[] ClearPrinter(PrinterSettings settings)
        {
            return Encoding.GetEncoding(437).GetBytes(string.Format("\nN\nO\nQ{0},{1}\nq{2}\nS{3}\nD{4}\nZB\nJF\nI8,{5:x},{6:000}\n", settings.Length + 10, 25,
                                                                    settings.Width + settings.AlignLeft, settings.PrintSpeed, settings.Darkness,
                                                                    (int)Codepage8.DOS_437, (int)Codepage8KDU.USA));
        }
        ...
        {
            string encodedReadable = readable
                ? "B"
                : "N";
            return Encoding.GetEncoding(437).GetBytes(string.Format("B{0},{1},{2},{3},{4},{5},{6},{7},\"{8}\"\n", left + settings.AlignLeft,
                                                                    top + settings.AlignTop, (int)rotation, barcode.P4Value, barcode.BarWidthNarrow,
                                                                    barcode.BarWidthWide, height, encodedReadable,
                                                                    barcodeData));
        }
            ...
            int horizontalMult, int verticalMult, bool isReverse, string text, PrinterSettings settings)
        {
            return Encoding.GetEncoding(437).GetBytes(string.Format("A{0},{1},{2},{3},{4},{5},{6},\"{7}\"\n", left + settings.AlignLeft,
                                                                    top + settings.AlignTop, (int)rotation, (char)font, horizontalMult, verticalMult, isReverse
                                                                        ? "R"
                                                                        : "N",
        ...
        public static byte[] EPL_Align(PrinterSettings p)
        {
            ...
            res.AddRange(TextWrite(5, 20, ElementDrawRotation.NO_ROTATION, ZebraFont.STANDARD_NORMAL,
                                   1, 1, false, "0", p));
            res.AddRange(LineWriteBlack(5, 40, 1, 20, p));
            res.AddRange(TextWrite(10, 40, ElementDrawRotation.NO_ROTATION, ZebraFont.STANDARD_NORMAL,
                                   1, 1, false, "5", p));
            res.AddRange(LineWriteBlack(10, 60, 1, 20, p));
            res.AddRange(TextWrite(15, 60, ElementDrawRotation.NO_ROTATION, ZebraFont.STANDARD_NORMAL,
                                   1, 1, false, "10", p));
            res.AddRange(LineWriteBlack(15, 80, 1, 20, p));
            res.AddRange(TextWrite(20, 80, ElementDrawRotation.NO_ROTATION, ZebraFont.STANDARD_NORMAL,
                                   1, 1, false, "15", p));
            res.AddRange(LineWriteBlack(20, 100, 1, 20, p));
            res.AddRange(TextWrite(25, 100, ElementDrawRotation.NO_ROTATION, ZebraFont.STANDARD_NORMAL,
                                   1, 1, false, "20", p));

            res.AddRange(LineWriteBlack(40, 0, 20, 1, p));
            res.AddRange(TextWrite(40, 5, ElementDrawRotation.NO_ROTATION, ZebraFont.STANDARD_NORMAL,
                                   1, 1, false, "0", p));
            res.AddRange(LineWriteBlack(60, 5, 20, 1, p));
            res.AddRange(TextWrite(60, 10, ElementDrawRotation.NO_ROTATION, ZebraFont.STANDARD_NORMAL,
                                   1, 1, false, "5", p));
            res.AddRange(LineWriteBlack(80, 10, 20, 1, p));
            res.AddRange(TextWrite(80, 15, ElementDrawRotation.NO_ROTATION, ZebraFont.STANDARD_NORMAL,
                                   1, 1, false, "10", p));
            res.AddRange(LineWriteBlack(100, 15, 20, 1, p));
            res.AddRange(TextWrite(100, 20, ElementDrawRotation.NO_ROTATION, ZebraFont.STANDARD_NORMAL,
                                   1, 1, false, "15", p));
            res.AddRange(LineWriteBlack(120, 20, 20, 1, p));
            res.AddRange(TextWrite(120, 25, ElementDrawRotation.NO_ROTATION, ZebraFont.STANDARD_NORMAL,
                                   1, 1, false, "20", p));
