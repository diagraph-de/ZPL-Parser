...
namespace ZebraParser.Parser.Commands
{
    public partial class EPLCommands
    {
        ...
        public static byte[] GraphicDirectWrite(int left, int top, string bitmapName, PrinterSettings settings)
        {
            ...
            List<byte> res = new List<byte>();
            int byteWidth = bmp.Width % 8 == 0
                ? bmp.Width / 8
                : bmp.Width / 8 + 1;
            res.AddRange(Encoding.GetEncoding(437).
                                 GetBytes(string.Format("GW{0},{1},{2},{3},", left + settings.AlignLeft, top + settings.AlignTop, byteWidth, bmp.Height)));
