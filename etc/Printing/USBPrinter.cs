    ...
    public class UsbPrinterConnector
    {
        ...
        public int Send(byte[] buffer, int offset, int count)
        {
            ...
            if (count > 4096)
            {
                ...
                while (current < count)
                {
                    int newcount = current + 4096 > count
                        ? count - current
                        : 4096;
        ...
        public static Dictionary<string, string> EnumDevices()
        {
            ...
            using (RegistryKey nameKey = Registry.LocalMachine.OpenSubKey("SYSTEM\\CurrentControlSet\\Control\\Print\\Printers", false))
            {
                foreach (string printer in nameKey.GetSubKeyNames())
                {
                    try
                    {
                        using (var subKey = Registry.LocalMachine.OpenSubKey("SYSTEM\\CurrentControlSet\\Control\\Print\\Printers\\" + printer))
                        {
                            var objPortValue = subKey.GetValue("Port");
                            if (objPortValue == null)
                                continue; //If it is null then don't move forward with it as .ToString() Raises Object Reference Not Set to an Instance Error.
                            portValue = objPortValue.ToString();
                    ...
                    {
                        //do nothing and move on to next entry
                    }
                }
                nameKey.Close();
            }
            ...
            using (RegistryKey regKey =
                    Registry.LocalMachine.OpenSubKey("SYSTEM\\CurrentControlSet\\Control\\DeviceClasses\\" + GUID_DEVICEINTERFACE_USBPRINT.ToString("B"),
                                                     false))
            {
                foreach (string sub in regKey.GetSubKeyNames())
                {
                    try
                    {
                        ...
                        {
                            using (var subKey =
                                    Registry.LocalMachine.OpenSubKey(string.Format("SYSTEM\\CurrentControlSet\\Control\\DeviceClasses\\{0:B}\\{1}\\#",
                                                                                   GUID_DEVICEINTERFACE_USBPRINT, sub)))
                            {
                                var objSymbolicLink = subKey.GetValue("SymbolicLink");
                                if (objSymbolicLink == null)
                                    continue; //If it is null then don't move forward with it as .ToString() Raises Object Reference Not Set to an Instance Error.
                                path = objSymbolicLink.ToString();
                                subKey.Close();
                            }

                            using (var subKey =
                                    Registry.LocalMachine.
                                            OpenSubKey(string.Format("SYSTEM\\CurrentControlSet\\Control\\DeviceClasses\\{0:B}\\{1}\\#\\Device Parameters",
                                                                     GUID_DEVICEINTERFACE_USBPRINT, sub)))
                            {
                                var objPortNumber = subKey.GetValue("Port Number");
                                if (objPortNumber == null)
                                    continue; //If it is null then don't move forward with it as .ToString() Raises Object Reference Not Set to an Instance Error.
                                if (int.TryParse(objPortNumber.ToString(), out portNumber))
                                {
                                    if (printerNames.ContainsKey(portNumber))
